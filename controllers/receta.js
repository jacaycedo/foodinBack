...
async function getRecetas() {
  const recetas = await db().collection(NOMBRE_COLLECCION).find({}).toArray()
...
async function deleteRecetas() {
  const x = await db().collection(NOMBRE_COLLECCION).remove({})

  return 'Eliminadas'
}

async function getRecetaByTitulo(tit) {
  const receta = await db().collection(NOMBRE_COLLECCION).findOne({nombre: tit})
  return receta
}
...
async function insertReceta(receta, req) {
  ...
  await db().collection(NOMBRE_COLLECCION).insertOne(newReceta)
  return
}
